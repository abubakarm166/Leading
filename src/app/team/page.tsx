    ...
    <div className="bg-[#C5D3DD] flex flex-row items-start space-x-10 overflow-x-scroll py-4 lg:py-[26px] px-5 lg:px-[100px] mt-[30px]">
      {team.length > 0 &&
        team.map((item) => (
          <Reveal
            key={item.id}
            className={`min-w-[160px] max-w-[160px] cursor-pointer ${item?.id === activeMember?.id ? "opacity-100" : "opacity-50"}`}
            delay={0.2 * item.id}
          >
            <div onClick={() => handleOnClick(item)}>
              <div
                className={`bg-white w-[160px] h-[150px] ${item?.id === activeMember?.id ? "border-[3px] border-primary shadow-xl" : ""} rounded-[20px] relative flex items-end justify-center`}
              >
                <Image
                  src={item.img}
                  width={200}
  ...
  return (
    <main className="bg-primary-bg">
      ...
      <TeamList
        ...
        team={team}
        setActiveMember={(item) => setActiveMember(item)}
      />
      <ContactUs />
      <Footer />
