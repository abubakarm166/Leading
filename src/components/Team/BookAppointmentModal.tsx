...
const BookAppointmentModal: React.FC<Props> = ({ isOpen, member, onClose }) => {
  ...
  const [typeSelected, setTypeSelected] = useState<"borrower" | "broker">("borrower");
  ...
  return (
    <Modal open={isOpen} onClose={onClose} classNames={{ modal: "w-[90%] lg:min-w-[960px] bg-white rounded-[20px]" }}>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:space-x-[30px] space-y-[30px] lg:space-y-0">
          <div>
            ...
            <div className="mt-4 bg-[#FFF3EA] px-[10px] py-2 flex flex-row items-center space-x-[14px]">
              ...
              <p className="font-gilroy-medium font-medium text-[15px]">All times are in GMT (United Kingdom)</p>
            </div>
          </div>
          <div className="flex-1">
            <Image src={member?.img} width={91} height={80} alt="member" className="w-[91px] h-[80px] object-contain" />
            <p className="font-gilroy-bold font-bold text-[30px] mt-[10px]">
              {member?.firstName} {member?.lastName}
            </p>
            <p className="font-gilroy-regular font-extralight text-[16px]">{member?.role}</p>
            <p className="mt-2 lg:mt-10 font-league-spartan font-semibold text-[20px]">You are</p>
