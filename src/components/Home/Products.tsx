...
/** Repeated so the row overflows even on very wide screens; loop resets each segment. */
const LOOP_SEGMENTS = 4;
const LOOP_PRODUCTS = Array.from({ length: LOOP_SEGMENTS }, () => PRODUCTS).flat();
    ...
    const tick = () => {
      if (!pausedRef.current) {
        ...
        if (max > 0) {
          ...
          const loopWidth = el.scrollWidth / segments;
          if (loopWidth > 0 && el.scrollLeft >= loopWidth - 1) {
        ...
        <div className="font-gilroy-regular font-extralight max-w-full mt-5 lg:mt-0 lg:max-w-[70%]">
          ...
          >
            <div
              ...
              className="flex flex-row items-center space-x-5 overflow-x-auto overflow-y-hidden [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
